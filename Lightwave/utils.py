...
from lwsdk.pris.modeler import init
...
def readUasset(file_h):
	...
	for y in range(stringCount):
		...
		size -= 1
		name = struct.unpack("%ss"%size,file_h.read(size))[0]
...
class path_wrangler():
	...
	def get_files(self):
		...
		self.data['tex_dir'] = base_dir.replace('\\', '\\\\') + '\\\\Texture\\\\'
		
		mat_d = os.listdir(self.data['mat_dir'])
		tex_d = os.listdir(self.data['tex_dir'])
		
		self.data['mat_file_list'] = []
		self.data['tex_file_list'] = []
		uassetFilter(self.data['mat_file_list'], mat_d)
		uassetFilter(self.data['tex_file_list'], tex_d)
...
#points
def v_f(x, va, meo):
	va.append(meo.addPoint(meo.state, x))

#Faces
def f_f(x, va, meo, y):
	meo.addPoly(
		meo.state,
		lwsdk.LWPOLTYPE_FACE,
		None,
		y[0],
		(va[x[0]], va[x[1]], va[x[2]])
	)

#UVs
def uv_f(p, uv, i, meo, name):
	meo.pntVMap(
		meo.state,
		p,
		lwsdk.LWVMAP_TXUV,
